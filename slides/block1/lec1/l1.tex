...
\newcommand{\taskInterleaveFirst}{1.2}
\newcommand{\taskInterleaveSecond}{1.3}
\newcommand{\taskAmdhal}{1.4}
\newcommand{\taskEmpire}{1.5}
\newcommand{\taskJstack}{1.6}
...
\questiontime{Multitasking, time-sharing, preemption, round-robin. We are going to talk about ...}

\begin{section}{Scheduler}
\showTOC

\begin{frame}[t]{Scheduler}
\framesubtitle{Reminders}

\begin{itemize}
    \item Ivan Ugliansky, ''Scheduling: from FCFS to EEVDF'' {\tiny\url{https://github.com/Svazars/parallel-programming/blob/main/slides/pdf-extra/algo_plus_ose_schedulers.pdf}}

    \item ''Operating Systems: Three Easy Pieces'', ''Scheduling: Introduction'' {\tiny\url{https://pages.cs.wisc.edu/\%7Eremzi/OSTEP/cpu-sched.pdf}}
    
    \item {\tiny\url{https://en.wikipedia.org/wiki/Process_(computing)}}
\end{itemize}
\end{frame}
...
\begin{center}
\includegraphics[width=0.7\textwidth]{./pics/full_process_pic.png}
\end{center}
...
\begin{itemize}
    \item ''knows and owns'' every unit of scheduling (thread, process)
...
Requires \texttt{user-space -> kernel space} transition.
...
\begin{itemize}
    ...
    \item thread/process priority    
    \item ''price'' of CPU migration 
    \item occupation of other processors
    \item ...
\end{itemize}

It is hard to trust timings\footnote{\url{https://shipilev.net/blog/2014/notrusting-nanotime}}\textsuperscript{,}\footnote{\url{https://pzemtsov.github.io/2017/07/23/the-slow-currenttimemillis.html}}.
...
In real environments it is \textbit{always} a hybrid solution\footnote<3->{The long road to lazy preemption: \url{https://lwn.net/Articles/994322/}}.
...
\questiontime{OS scheduler schedules \textbf{what}? Scheduler manages which OS \textbf{resource}?}
...
\begin{frame}[fragile]{Your first homework}

\pause
Read the docs
\begin{itemize}
    \item \tiny\url{https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/lang/Thread.html}
    \item \tiny\url{https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/lang/Thread.html#start()}
    \item \tiny\url{https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/lang/Thread.html#join()}
\end{itemize}

\pause Yes, we use JDK 11 for all homeworks. \pause Yes, in \the\year{}.

\pause \item Anytime you see \texttt{javase.docs} URL on the lecture slides
\begin{itemize}
    \pause \item I am free to ask you about corresponding Java API during the exam        
\end{itemize}

\pause
{\tiny\url{https://github.com/Svazars/parallel-programming/blob/main/hw/block1/1.1/readme.markdown}}
\begin{homeworkmail}{Task \taskExceptions}
    Create 3 Java programs for the following cases, explain results in several sentences.
}
\end{homeworkmail}
...
\questiontime{Why would somebody use Java for parallel programming course, not C or C++?}
...
\begin{section}{Multitasking and threading models}
\showTOC

\begin{frame}[t]{Threading models}

1:1 model
\begin{itemize}
    \item \texttt{N} preemptible tasks
    \item Every task is allocated on designated computing agent
    \pause
    \item Examples:
    \pause
    \begin{itemize}
        \item 1 Java thread -- 1 OS thread
        \item 1 OS thread -- 1 CPU        
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item \texttt{N} preemptible tasks
    \item Multiplexed over \texttt{M} computing agents. (\texttt{1 < M < N})
    \pause
    \item Examples:
    \pause
    \begin{itemize}
        \item thread pools (Lecture~\syncPrimitivesNum)
        \item green threads, coroutines (Lecture~\userSpaceThreadingNum)                
    \end{itemize}    
\end{itemize}

\end{frame}

\begin{frame}[t]{N:1 threading model}
...
Single-core OS
\begin{itemize}
    \item \texttt{N} running processes
    \item Single CPU
\end{itemize}
...
\begin{itemize}
    \item \texttt{N} running Java threads
    \item Each Java thread mapped to corresponding OS thread (\texttt{1:1})
    \item OS threads run on single CPU (\texttt{N:1})
\end{itemize}

\pause
Useful \textbf{model} for discussing possible behaviours of concurrent programs

\end{frame}


\begin{frame}[t]{Threading model: not enough}
...
\begin{itemize}
    \item guarantees to memory update visibility?
    \item expectations about scheduling policy?
    \item cross-thread interactions ordering?
\end{itemize}
\end{frame}
...
\pause
This is \textbf{simplified} model of concurrent execution. We will use it up to Lecture~\foundationsNum.

\end{frame}

\questiontime{What is the difference between \texttt{N:1} threading model and describing concurrent system using \texttt{interleavings}?}

\input{parts/execution_trace_workers.tex}

\begin{frame}{Interleaving model: key concepts}

Concurrent execution trace:
\begin{itemize}
    \item All operations inside thread ordered sequentially
    \item All operations across all threads are totally ordered on timeline
    \item Single deterministic result of execution    
\end{itemize}
...
\questiontime{single-threaded program that have many execution traces?}
\questiontime{concurrent program with many execution traces but only one result of execution?}
\questiontime{concurrent program with infinite number of execution traces?}

\begin{frame}[fragile]{Interleaving model: homework}

\begin{homeworkmail}{Task \taskInterleaveFirst.}
    For given multithreaded program and execution result provide either 
    \begin{itemize}
        \item concurrent execution trace (\texttt{A.1->A.2->B.1 ...}) or 
        \item proof of impossibility
    \end{itemize}
}
\end{homeworkmail}
{\tiny\url{https://github.com/Svazars/parallel-programming/blob/main/hw/block1/1.2/readme.markdown}}

\newline
\begin{homeworkmail}{Task \taskInterleaveSecond.}
    For given multithreaded program:
    \begin{itemize}
        \item provide all possible execution results
        \item provide only one execution trace per result
        \item prove exhaustiveness of your answer
    \end{itemize}   
}
\end{homeworkmail}
{\tiny\url{https://github.com/Svazars/parallel-programming/blob/main/hw/block1/1.3/readme.markdown}}

\end{frame}


\begin{frame}[t]{Our main enemy through the course}

\pause
Non-determinism\pause: concurrent program may have \textbf{many} execution traces.    

\begin{itemize}
    \pause \item Hard to reason about all possible memory states
    \pause \item Hard to define and guarantee program invariants
    \pause \item Hard to reproduce erroneous behavior
    \pause \item Hard to verify concurrent bug is fixed
    \pause \item Hard to prove correctness
\end{itemize}

\pause
Solution: \pause study hard!
\begin{itemize} 
    \pause \item Write and debug concurrent programs (Lectures~\basicNum-\extraBasicsNum)
    \pause \item Use formalization for concurrent execution (Lectures~\foundationsNum-\atomicsNum)
    \pause \item Go to hardware-level (Lecture~\cacheCoherencyNum)
    \pause \item Go to semantic level (Lecture~\langMMNum)
    \pause \item ... and beyond  (Lectures~\advancedConcurrencyNum+)
\end{itemize}
\end{frame}

\begin{frame}{Concurrency: tips and tricks}

\begin{itemize}
    \pause \item If concurrent tests pass on your local machine
    \begin{itemize}
        \pause \item no guarantee your program is correct.
    \end{itemize}

    \pause \item If some concurrent test fails
    \begin{itemize}
        \pause \item maybe there is a bug in test itself.
    \end{itemize}

    \pause \item If interleaving model explains a bug
    \begin{itemize}
        \pause \item you must fix it. \pause \textbf{Always}.
    \end{itemize}

    \pause \item If interleaving model proves correctness of your program
    \begin{itemize}
        \pause \item still run stress-tests.
    \end{itemize}
\end{itemize}
\end{frame}
...
\showTOC
 ...
 \begin{frame}{Concurrent building blocks you know so far}
 
 \begin{enumerate}
     \item Create new thread and \texttt{start} it
 ...
 \begin{itemize}
     ...
     \begin{itemize}
         ...
         \item proof-of-work cryptocurrency
 ...
 \pause
 {\tiny\url{https://github.com/Svazars/parallel-programming/blob/main/hw/block1/1.4/readme.markdown}}
 ...
 \pause
 {\tiny\url{https://github.com/Svazars/parallel-programming/blob/main/hw/block1/1.5/readme.markdown}}
...
% TODO: separate optional homework?
% 
% \begin{frame}[fragile]{Data race}
...
\pause
{\tiny\url{https://github.com/Svazars/parallel-programming/blob/main/hw/block1/1.6/readme.markdown}}
...
\begin{itemize}
    \item Agents with different speed could communicate with each other via shared memory
    \item Parallel and concurrent execution could look similar and could happen in the same system
    \begin{itemize}
        \item Concurrency: tasks may be interrupted in pre-defined places or arbitrary
        \item Parallel: independent tasks may be executed simultaneously on different cores
    \end{itemize}
    \item Threads are parts of OS processes and are minimal unit of work for scheduler
    \item There are different threading models: \texttt{1:1}, \texttt{N:1}, \texttt{N:M}
    ...
    \item Whenever you coordinate threads, you encounter non-determinism, race conditions, data races, visibility issues, deadlocks, priority inversion
\end{itemize}
...
\texttt{Lecture 1, Tasks 1.*:} {\tiny\url{https://github.com/Svazars/parallel-programming/blob/main/hw/block1}}
